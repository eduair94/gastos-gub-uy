/**
 * Deterministic signature of a call's supported pliego documents (PDF/Word).
 * Pliegos can be MODIFIED after
 ...
 *
 * Signature = sorted `url@datePublished` of every supported document, hashed to a short
 ...
 */
import { isSupportedPliegoDocument } from "../services/pliego-extractor";
...
/** Empty string when there are no supported documents (so "no pliego" is its own value). */
export function pliegoDocsSignature(documents: DocLike[] | undefined): string {
  const parts = (documents ?? [])
    .filter((d) => isSupportedPliegoDocument(d))
    .map((d) => {
