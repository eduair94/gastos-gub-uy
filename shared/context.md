...
| [models/judicial_spending.ts](models/judicial_spending.ts) · [judicial-objects.ts](judicial-objects.ts) | OPP budget credit for the expense objects whose official name is a judgment, a judicial settlement or a health injunction. The taxonomy is a CODE table, never a text match: eleven unrelated objects share the words, and the same object is renamed between years. `judicial_spending_years` carries the MEASURED coverage of each OPP file, because 2013-2015 and 2017-2018 are fragments and 2019-2021 publish no execution. Never read `ejecutado` without its year's `executionAvailable`. |
