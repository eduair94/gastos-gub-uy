...
| [utils/heavy-export.ts](utils/heavy-export.ts) | Shared per-worker export slot plus bounded CSV/JSON/vCard and ExcelJS streaming writers. Both 50k-row contact export routes use it so downloads cannot fill a worker heap. |
...
| [routes/rss/anomalies.xml.get.ts](routes/rss/anomalies.xml.get.ts) + [utils/anomaly-rss.ts](utils/anomaly-rss.ts) | Public `GET /rss/anomalies.xml`: source-procurement-date newest-first via an indexed `releases.id` lookup, with the same severity/AI/category/rubro/buyer/supplier/product/year filters as the human list. Each full path+query is Redis-SWR cached. |
...
| [api/contracts/](api/contracts/) | `[id].get.ts`, `stats.get.ts`, `filters.get.ts`, `[id]/features.get.ts`, `item-features/batch.post.ts` (live gov HTML scrape; `MAX_ITEMS=25`, `CONCURRENCY=5`, `BUDGET_MS=12000`). | Its `[id]` handler also attaches `bidders` — who else offered, from `acta_bidders`, present on only ~4% of contracts because the OCDS feed never carries it (see shared/acta-bidders.ts); a miss is null and must not render as "no competition".
...
| [api/analytics/](api/analytics/) | `anomalies.get.ts` (the `sourceDate` sort joins indexed `releases.id` before pagination; other sorts batch-enrich their visible rows) + `anomalies/{[id],facets,stats,load-error-stats}` + `anomalies/[id]/feedback.{post,delete}`, `category-distribution`, `top-buyers`, `top-suppliers`, `supplier-types`, `products(.get + [code])`, `provider-anomalies`, `organism-groups`, `intendencias`, `party-comparison`, `spending-trend` (25 precomputed year docs, whole series in one response), `dei-signals`, `omisos` (JUTEP roster; the cédula is MASKED on the way out — public in the source, but full identity documents do not belong on a high-traffic page), `sanciones` (proveedores del Estado sancionados por Defensa del Consumidor; cada firma de la página viene con sus sanciones individuales — fecha, tipo, motivo, monto — en UN $in sobre udeco_sanctions, no una consulta por fila; reads the precomputed cross-reference — the RUT join needs a 12-digit normalisation no index can serve), `integrity-signals` (señales de gestión, one precomputed doc per `buyer.id`; plain indexed find — the measurements behind it are COLLSCANs and live in `refresh-integrity-signals`, and its `signal`/`level` filter uses `$elemMatch` so both conditions must hold on the SAME array entry). |
...
- **Contact exports have two separate guards.** `rateLimit.ts` limits each edge IP/API key to 5/min, while [utils/heavy-export.ts](utils/heavy-export.ts) permits one active export per pm2 worker across both `/api/contacts/export` and `/api/contactos/export`. Both routes use Mongo cursors and 250-row serializers; do not replace them with `.lean()` into one 50k-row array or ExcelJS `writeBuffer()`.
...
- **Limiter state is still per-process** (`rateLimit.ts:11`), so two pm2 workers make its effective ceiling 2× configured. The heavy-export slot is deliberately per-worker too (maximum two exports box-wide). The Nitro response cache is shared through Redis, but [utils/rates.ts](utils/rates.ts):11, [api/pauta.get.ts](api/pauta.get.ts):25, and the curros/recopilatorios module caches remain per-worker.
