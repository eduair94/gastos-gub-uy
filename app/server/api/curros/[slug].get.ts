...
import { curroToQueryParams, getCurroDef, listCurroDefs } from '../../utils/curros'
...
/**
 * One curro, resolved live. Given a slug, it looks up the case, builds the SAME
 * match the explorer would, and returns the case's cross-referenced total,
 * supplier mix, rubro mix, a year trend and the ledger of contracts — plus the
 * legal status and the sources that make it a case.
 *
 * Totals exclude the implausible-quantity artefacts (see stats.get.ts) so one
 * corrupt source record can't blow up a case total.
 */
...
export default defineEventHandler(async (event) => {
  ...
  const def = getCurroDef(slug)
  if (!def) {
    throw createError({ statusCode: 404, statusMessage: 'Curro not found' })
  }
  ...
  const facet: PipelineStage = {
    $facet: {
      ...
      // own currency. Single-currency events only; a mixed-currency case would
  ...
  // Related = the other cases, for lateral navigation.
  const related = listCurroDefs()
    .filter(c => c.slug !== def.slug)
    .map(c => ({ slug: c.slug, emoji: c.emoji, status: c.status, es: { title: c.es.title }, en: { title: c.en.title } }))
  ...
  return {
    ...
    data: {
      ...
      related,
      meta: {
        crossReferenceBasis: 'the DB set is what the state spent with the entities named in the case — a cross-reference, not a measure of wrongdoing',
