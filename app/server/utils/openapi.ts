    ...
    '/api/contracts': {
      get: {
        ...
        description:
          ...
          + '**Commas inside a value:** many names contain one — e.g. `Administración Nacional de '
          + 'Combustible, Alcohol y Portland`. Escape it as `%2C` *before* URL-encoding the parameter '
          + '(so it reaches the API as `%2C`, i.e. `%252C` on the wire) and the value stays whole. '
          + 'A bare comma is still read as a separator, but for `buyers`, `suppliers` and `category` '
          + 'the unsplit value is also tried as an exact match, so an unescaped name still resolves.\n\n'
