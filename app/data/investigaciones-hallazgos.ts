 ...
 *
 * POR QUÉ ESTA SECCIÓN EXISTE Y CÓMO NO SE PUEDE ESCRIBIR. Reúne hallazgos que cumplen tres
 * condiciones a la vez: salieron de medir este corpus, no tienen cobertura previa en prensa, y
 * ponen en juego una norma concreta. Esa tercera condición es la que obliga a la disciplina:
 * nombrar una norma NO es acusar de violarla.
 *
 * Cada ficha lleva cuatro campos, y ninguno es opcional:
 *   - `measured`: el hecho, con su número y su fecha de medición. Es lo único que afirmamos.
 *   - `norm` + `normCite`: la norma que el hecho pondría en juego SI se confirmara.
 *   - `missing`: qué falta para poder afirmarlo. Es el campo que impide que la ficha se lea como
 *     una condena, y el que más se va a querer recortar. No se recorta.
 *   - `answers`: quién tiene que responder. Un hallazgo sin destinatario es un chisme.
 *
 * FORMA DEL ARCHIVO: cada ficha es AUTOCONTENIDA y trae sus dos idiomas adentro. Antes el texto
 * vivía partido entre este array y el objeto de contenido, así que sumar una ficha obligaba a
 * tocar tres lugares y a mantener sincronizadas dos listas de claves. La sección se llena por
 * tandas verificadas, y una tanda tiene que ser un append.
 *
 * Lo que NO va acá: nada que ya haya publicado la prensa (el caso ANTEL/Tribunal de Cuentas, por
 * ejemplo, vive en la investigación de competencia porque es antecedente, no hallazgo nuestro), y
 * nada que no se pueda medir de nuevo con un comando.
 */
...
export interface Finding {
  ...
  reproduce: string
  /** Cuándo se midió, para que el lector sepa contra qué foto está leyendo. */
  measuredOn: string
  es: FindingText
  en: FindingText
}
...
export const FINDINGS: Finding[] = [
  {
    ...
    reproduce: 'npx tsx tests/unit/competencia-aparente.verify.ts',
  ...
  },
  {
    key: 'im',
    path: '/investigaciones/competencia-aparente',
    reproduce: 'npx tsx src/jobs/refresh-bidder-competition.ts --dry-run',
    measuredOn: '2026-08-13',
    es: {
  ...
  {
    key: 'unitario',
    reproduce: 'npx tsx tests/unit/sobreprecio-organismo.verify.ts',
    measuredOn: '2026-08-13',
    es: {
  ...
  {
    key: 'moneda',
    reproduce: 'npx tsx tests/unit/moneda-nula.verify.ts',
    measuredOn: '2026-08-14',
    es: {
      title: 'Compras en dólares contabilizadas como si fueran pesos',
      measured: 'Cuando el feed publica una línea sin moneda, el registro la cuenta como pesos uruguayos. Lo verificamos contra la ficha oficial de la compra 1126716, que dice literalmente "U$S B 27.500,00" por unidad y seis unidades —USD 165.000— y que en el corpus figura como 0,17 millones de pesos: unas cuarenta veces menos. No es un caso aislado: 39.607 adjudicaciones desde 2015 (el 4,1%) tienen al menos un ítem sin moneda declarada.',
      norm: 'Ninguna: es un defecto del dato publicado, no una irregularidad de la compra. Pero tiene consecuencia pública, porque el error no es aleatorio: castiga sistemáticamente a quien cotiza en dólares —vehículos, equipamiento médico, informática importada— frente a quien cotiza en pesos, y eso deforma cualquier ranking de proveedores construido sobre el registro, incluido el nuestro.',
      normCite: 'Régimen de publicidad del portal de Compras Estatales; la moneda es parte del dato que se publica.',
      missing: 'La moneda correcta, que no está en el feed. Se puede leer una por una en la ficha oficial de cada compra, pero son casi cuarenta mil. Mientras tanto, lo honesto es decir que esas compras están mal contabilizadas y no usarlas para rankings.',
      answers: 'La Agencia Reguladora de Compras Estatales, que publica el feed sin el campo; y nosotros, que hasta ahora las sumábamos como pesos sin advertirlo.',
    },
    en: {
      title: 'Purchases in dollars counted as if they were pesos',
      measured: 'When the feed publishes a line with no currency, the record counts it as Uruguayan pesos. We checked it against the official page of purchase 1126716, which reads literally "U$S B 27,500.00" per unit for six units — USD 165,000 — and which the corpus stores as 0.17 million pesos: about forty times less. It is not an isolated case: 39,607 awards since 2015 (4.1%) have at least one item with no declared currency.',
      norm: 'None: it is a defect in the published data, not an irregularity in the purchase. But it has public consequences, because the error is not random: it systematically penalises those who quote in dollars — vehicles, medical equipment, imported IT — against those who quote in pesos, and that distorts any supplier ranking built on the record, including ours.',
      normCite: 'Publicity regime of the state procurement portal; currency is part of the published datum.',
      missing: 'The correct currency, which is not in the feed. It can be read one by one on each purchase\'s official page, but there are almost forty thousand. In the meantime, the honest thing is to say those purchases are miscounted and not to use them for rankings.',
      answers: 'The state procurement agency, which publishes the feed without the field; and ourselves, who had been adding them up as pesos without warning anyone.',
    },
  },
]

export function findingText(f: Finding, locale: string): FindingText {
...
export const HALLAZGOS_CONTENT = {
  es: {
    ...
    fileScope: '{n} hallazgos',
    fileSource: 'medidos sobre el corpus',
  ...
  en: {
    ...
    fileScope: '{n} findings',
    fileSource: 'measured on the corpus',
