...
| [pages/analytics/](pages/analytics) | Detection/analysis family, hub at `index.vue`: `anomalies.vue` + `rss-anomalias.vue` (personal RSS builder), `unexplained.vue`, `errores-carga.vue` (reportable load errors + ReportErrorDialog), `proveedores-anomalias.vue`, `intendencias.vue`, `organismos.vue`, `partidos.vue` (choropleth), `mapa.vue` (treemap), `como-reportar.vue`. |
