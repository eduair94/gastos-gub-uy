...
| [pages/colaboradores.vue](pages/colaboradores.vue) | Credits: people, data sources, free software, how to contribute. Holds NO names — everything comes from `data/contributors.json` through `utils/contributors.ts`. **Adding a contributor is an edit to that JSON alone**: it carries both locales inline, so the locale files are not involved. |
...
| **Gutter**: the page gutter is `.u-container`'s `padding-inline` and nothing else sets it. A scoped `padding` shorthand on that SAME element is more specific and wins, so `padding: var(--s-7) 0 var(--s-9)` zeroes the gutter and the page goes edge-to-edge on a phone. Write vertical rhythm as `padding-block`. | [assets/scss/main.scss:267](assets/scss/main.scss), [pages/analytics/genero.vue](pages/analytics/genero.vue) |
| **Spacing scale stops at `--s-9`.** `var(--s-10)`+ makes the whole declaration invalid, so it is dropped and the element silently loses that padding/gap. No error, no warning. | [assets/scss/_tokens.scss:100-108](assets/scss/_tokens.scss) |
| **Two sibling tags have NO space between them** — Vue condenses the newline. `<span>{{ name }}</span><MandateChip/>` renders welded, with no text node left to space later. Wrap a value plus its trailing chip in `.chip-row chip-row--baseline`. | [assets/scss/main.scss](assets/scss/main.scss) `chip-row`, [components/MandateChip.vue](components/MandateChip.vue) |
...
| A new investigation page = static chrome over a typed module in `data/` (never live queries) + an entry in the `investigaciones` group's `casos` section. | [utils/nav.ts](utils/nav.ts) |
...
- **Mobile layout defects here are SILENT** — the page renders, it just renders wrong, and only under ~640px, which no typecheck, lint or SSR smoke test sees. [scripts/check-layout-guards.mjs](../scripts/check-layout-guards.mjs) (`npm run check:layout`, also in `prebuild`) catches the four that have actually shipped: gutter-killing `padding` shorthand, off-scale `--s-10+`, welded chip siblings in a `#cell:` slot, and `<v-pagination>`. It is a text scan; it cannot see a layout that is merely ugly. For that, load the route in a real browser at 360px and assert `document.documentElement.scrollWidth <= innerWidth` and that no content box starts at `x = 0`.
...
- **The PWA has NO browser runtime caching by design** ([nuxt.config.ts](nuxt.config.ts), [service-worker/sw.ts](service-worker/sw.ts)): SSR pages and `/api` are never persisted in the browser. This is separate from Nitro's server-side Redis SWR for selected public GET APIs; that cache is bounded, shared by pm2 workers, and falls through on Redis failure. The SW is disabled in `nuxt dev` — test installability against a production preview.
