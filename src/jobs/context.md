...
| [lib/comprasestatales-total.ts](lib/comprasestatales-total.ts) | Parses "Monto Total de la Compra" off the official gov page, bound to the label's sibling `<li>`. `parseUyNumber`, `parseOfficialTotal`, `fetchOfficialTotal`, `detalleUrl`, `idCompraFromOcid`. The only defence against writing a wrong number as "verified". |
...
| [pliego/summarize.ts](pliego/summarize.ts) | `summarizeOpenCall()`: download pliego PDFs → extract text → Gemini → `open_calls.aiSummary`. Guardrail: user-facing deadlines always come from OCDS `tenderPeriod`, never from this summary. |
