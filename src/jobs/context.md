...
- **La escalera de modelos arranca en Claude (servidor 104), después Gemini, después Groq.** El escalón Claude sólo existe si `CLAUDE_AGENT_API_KEY` está en `.env`. Su cuota es de 200 llamadas por día, así que una tanda larga lo agota y sigue con Gemini. Los jobs que llamaban a Gemini de forma directa ahora usan `callStructured` de [shared/ai/structured.ts](../../shared/ai/structured.ts). El OCR de pliegos queda en Gemini: manda el PDF binario y el agente recibe texto.
- **Anomaly AI uses the shared Claude → Gemini → Groq model ladder** with streaming, a 45s inactivity timeout per model and a 180s overall budget per flag. A stuck model falls through instead of retrying the same timeout forever; a whole-ladder failure stays pending for the hourly retry lane.
