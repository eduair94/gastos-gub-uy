/**
 * Minimal Gemini REST client for the anomaly triage job.
 *
 ...
 * all the triage job needs: a bounded classification, not open chat.
 *
 * The API key is read by the caller from process.env (GEMINI_API_KEY) and passed
 * in — this module never touches the environment, so it can be unit-tested with a
 * stub key and a mocked fetch.
 */
 ...
 * window is ~60s. Returns the delay in ms, or null when absent/over the ceiling.
 */
function parseRetryDelayMs(body: string): number | null {
  ...
  return Number.isFinite(ms) && ms > 0 && ms <= MAX_HONORED_RETRY_MS ? ms : null;
}
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (isRetryableStatus(response.status) && attempt < maxRetries) {
          lastError = new Error(`Gemini ${response.status}: ${text.slice(0, 300)}`);
      ...
      const retryable = err.name === "AbortError" || err.name === "TypeError" || /Gemini (429|500|502|503|504)/.test(err.message);
      lastError = err;
