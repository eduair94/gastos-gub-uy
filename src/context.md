...
Everything that gets Uruguayan OCDS procurement data **into** Mongo lives here. This layer also builds the precomputed collections the Nuxt app reads. Nothing here serves HTTP to users — the real API is `app/server/api/**`.

One long-running process ([cronserver.ts](cronserver.ts), pm2) schedules 19 cron jobs. It spawns each heavy job as a child process from [jobs/](jobs/). A large part of this tree is dead legacy — the 2025 zip-scrape path, `src/api/**` — and the map marks it.
...
| path | purpose |
|---|---|
| [cronserver.ts](cronserver.ts) | Express scheduler + control plane. Anomaly AI runs two lanes: a nightly unlimited high/critical lane, and a bounded hourly retry/backfill lane over recent flags of any severity. `runJobProcess()` spawns the heavy work. |
| [uploaders/release-uploader-new.ts](uploaders/release-uploader-new.ts) | LIVE RSS/API ingestion → `releases`. 4 public methods (`:17-20`): `uploadReleasesFromWeb` `:34`, `uploadCurrentMonthFromWeb` `:119`, `uploadLastSevenDaysFromWeb` `:165`, `reconcileNonFinalReleases(monthsBack=5)` `:676`. Freshness diff `filterReleasesNeedingSync()` `:637`. |
...
| [populate-filters.ts](populate-filters.ts) | LIVE (spawned at the tail of the analytics cron). Rebuilds `filter_data` from `releases`. Sets `MONGO_SOCKET_TIMEOUT_MS` first (`:13-16`) — see Conventions. |
...
| [jobs/refresh-analytics.ts](jobs/refresh-analytics.ts) | Rebuilds `supplier_patterns`/`buyer_patterns` + compute-then-swap `spending_trends`/`top_entities`/`category_distribution`/`dashboard_metrics`. Replaces `populate-analytics.ts` + `precalculate-dashboard.ts` (header `:5-8`). |
...
| [jobs/refresh-organism-groups.ts](jobs/refresh-organism-groups.ts) · [jobs/refresh-dept-indicators.ts](jobs/refresh-dept-indicators.ts) | Monthly rollups → `organism_group_stats`, `dept_indicators`. See Gotchas. |
...
| [jobs/reconcile-award-amendments.ts](jobs/reconcile-award-amendments.ts) | Folds `ajuste_adjudicacion`/`awardUpdate` deltas into the base award release, recomputes `amount`, re-scores. Prints `RECONCILE_SUMMARY corrected=<n>` — see Conventions. |
| [jobs/correct-lumpsum-artifacts.ts](jobs/correct-lumpsum-artifacts.ts) | The ONLY thing that rewrites an inflated stored total, stamping `amount.verifiedOverride`. |
...
| [jobs/refresh-exchange-rates.ts](jobs/refresh-exchange-rates.ts) · [jobs/seed-historical-rates.ts](jobs/seed-historical-rates.ts) | `exchange_rates` monthly BCU averages (cron) / one-time 2000–2022 SOAP backfill. |
| [jobs/backfill-pliego-docs.ts](jobs/backfill-pliego-docs.ts) · [jobs/backfill-reiteracion-docs.ts](jobs/backfill-reiteracion-docs.ts) | One-off deterministic document-URL probes for the two feed gaps. |
| [jobs/enrich-suppliers.ts](jobs/enrich-suppliers.ts) · [jobs/enrich-supplier-contacts.ts](jobs/enrich-supplier-contacts.ts) | `supplier_enrichment` (category + one-liner) / `supplier_contacts` (cold-email Phase A). |
| [jobs/pliego-summary.ts](jobs/pliego-summary.ts) | Gemini pliego summaries cached on `open_calls.aiSummary`. |
| [jobs/analytics-pipeline.ts](jobs/analytics-pipeline.ts) | **LIBRARY, no imports.** The money rule for every aggregate: `MAX_PLAUSIBLE_RELEASE_UYU` `:47`, `AWARD_MATCH`, `FX_SCALE`, `UNWOUND_ITEM_UYU`. |
| [jobs/anomaly-stats.ts](jobs/anomaly-stats.ts) | **LIBRARY, no imports.** The whole estimator + every tuning constant (`Z_FLAG_THRESHOLD`, `MIN_BASELINE_N`, `MAX_REPORTED_Z`, …). Change severity behaviour here, not in the detector. |
...
| [jobs/campaign/](jobs/campaign/) | Cold-email: `enqueue.ts`, `send.ts`, `recipients.ts`, `suppression.ts`, `unsubscribe-core.ts`, `brevo-events.ts`, `open-calls-count.ts`. See Gotchas. |
...
| [api/](api/) (`app.ts`, `server.ts`, `routes/{analytics,buyers,contracts,dashboard,search,suppliers}.ts`) · [analytics-server.ts](analytics-server.ts) · [services/analytics-data-service.ts](services/analytics-data-service.ts) | Legacy Express REST API. Nothing in `ecosystem.config.js` starts it. |
...
- **Models come from `shared/models`, never defined here.** `import { ReleaseModel } from '../../shared/models'` ([uploaders/release-uploader-new.ts:1](uploaders/release-uploader-new.ts)). Two legacy files import `../app/server/utils/models` instead ([precalculate-dashboard.ts](precalculate-dashboard.ts), [quick-precalculate.ts](quick-precalculate.ts)) — don't copy that. [populate-filters.ts:3](populate-filters.ts) still connects via `../app/server/utils/database`; new code uses `shared/connection/database`.
- **Every write to `releases.amount` calls `hasVerifiedOverride(doc)` first** — from `shared/utils/verified-override` — and skips that field. All four writers do: [release-uploader-new.ts:431](uploaders/release-uploader-new.ts) and `:786`, [add-missing-amounts.ts:3](add-missing-amounts.ts), [uploaders/release-uploader.ts](uploaders/release-uploader.ts).
- **Write with `bulkWrite(ops, { ordered: false })` in slices.** Each op is `updateOne + upsert:true` filtered on `{ id: release.id }` — the OCDS release id, never `_id`. Stamp every doc with `sourceFileName` + `sourceYear`. The live path uses `sourceFileName: 'web'` plus the `rss*` fields.
...
- **Long jobs set `process.env.MONGO_SOCKET_TIMEOUT_MS = 30*60*1000` BEFORE the first `connectToDatabase()`** — see [populate-filters.ts:13-16](populate-filters.ts). The shared default is a 45s IDLE-socket timeout, tuned for the web app. It kills legitimate aggregations mid-flight.
...
- **Entrypoint shape:** `#!/usr/bin/env tsx` + a docblock stating WHY, then `main()`, then `if (require.main === module) { … }`. Only the `require.main` form is importable as a library — that is how `reconcile-award-amendments.ts` imports `rescoreReleaseIds` from `detect-anomalies.ts`.
...
- **Optional transports degrade to a logged no-op** when their env var is missing, instead of throwing ([services/mailer.ts](services/mailer.ts)). Keep that for any new transport.
- **tsconfig is strict with `exactOptionalPropertyTypes` + `noUnusedLocals`.** Use conditional spreads instead of assigning `undefined`.

## Gotchas

- **THE amount bug: [utils/amount-calculator.ts:175](utils/amount-calculator.ts)** — `const itemTotal = item.unit.value.amount * quantity`. The government sometimes puts a contract lump sum in the unit price. `amount.primaryAmount` then inflates by orders of magnitude, and the write stores it forever. Nothing in `src/` corrects it: the detector only *suppresses* (`isLineTotalArtifact`), the pipeline only *excludes* (`MAX_PLAUSIBLE_RELEASE_UYU`, [jobs/analytics-pipeline.ts:47](jobs/analytics-pipeline.ts)), and the only rewriter is [jobs/correct-lumpsum-artifacts.ts](jobs/correct-lumpsum-artifacts.ts) — which is neither scheduled nor npm-scripted.
- **Search [jobs/detect-anomalies.ts](jobs/detect-anomalies.ts) with `Read` or `grep -a`.** The file holds a literal NUL byte, inside `anomalyContentVersion` near `:231`. `file` reports "binary data" and ripgrep/grep return only "Binary file … matches", so the Grep tool silently finds nothing in it.
...
- **The two uploaders write differently on conflict.** Legacy `ReleaseUploader` uses a **pipeline** update that keeps stored `awards`/`amount` when the stored doc has strictly more awards. Live `ReleaseUploaderNew` does a plain `$set` of a fully rebuilt doc (`:464`), so a thin re-publish can overwrite a richer one. Pipeline-style `$set` also evaluates values as aggregation expressions, so every field needs a `$literal` wrapper or Mongo rejects empty nested objects like `buyer.contactPoint: {}`.
- **The uploader fetches exchange rates once per run.** It falls back to `FALLBACK_RATES` ([utils/amount-calculator.ts:46](utils/amount-calculator.ts)) silently on failure. A rate-API outage produces plausible-looking but wrong UYU totals, with no marker other than `exchangeRateDate`.
- **`calculateSimpleTotalAmounts()` ([utils/amount-calculator.ts:230](utils/amount-calculator.ts)) returns version 1 and skips conversion.** `createAmountUpdateQuery()` selects on `amount.version != 2`, so `add-missing-amounts` permanently re-queues anything it writes. Nothing live calls it — don't start.
...
- **lumpsum: `candidateMatchStage()` is a loose Mongo pre-filter** that cannot express the `maxPricedItems` cap. Every consumer must re-check each fetched doc with `isLumpsumSuspect()` before acting — contract in the module docblock ([jobs/lib/lumpsum-candidates.ts:23-29](jobs/lib/lumpsum-candidates.ts)). Known latent gap documented at `:49-54`: the Mongo currency `$in` is case-sensitive while `isLumpsumSuspect()` upper-cases.
...
| var | effect |
|---|---|
| `MONGODB_URI` | Mongo connection (via `shared/config.ts`, with `MONGO_URI` fallback; also read directly in `cronserver.ts` and `config/config.ts`). `.env` overrides shell env — see Gotchas. |
| `MONGO_SOCKET_TIMEOUT_MS` | Per-process idle-socket timeout; `populate-filters.ts` sets it to 30 min **if unset** ([:13-16](populate-filters.ts)). |
| `CRON_SERVER_PORT` | cronserver HTTP port, code default 3002 ([cronserver.ts:1298](cronserver.ts)). |
...
| `AI_TRIAGE_RPM` | Passed as `--rpm` to `score-anomalies-ai`, default 18 — see Gotchas. |
