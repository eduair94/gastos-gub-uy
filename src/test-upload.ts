...
async function testUploadAndVerification(): Promise<void> {
  ...
  try {
    ...
    const yearDistribution = await ReleaseModel.aggregate([{ $group: { _id: "$sourceYear", count: { $sum: 1 } } }, { $sort: { _id: 1 } }]);
    ...
    const sampleSources = await ReleaseModel.aggregate([{ $group: { _id: "$sourceFileName", count: { $sum: 1 } } }, { $limit: 10 }]);
