...
const proveedorUy = PROVIDERS.find(provider => provider.id === 'proveedoruy')
assert.ok(proveedorUy, 'ProveedorUY must be included')
assert.equal(proveedorUy.entryPaid?.currency, 'UYU')
assert.match(proveedorUy.entryPaid?.text ?? '', /^\$U /)
assert.equal(proveedorUy.features.whatsappTelegram, 'no')
assert.equal(proveedorUy.confidence, 'alta')
assert.match(proveedorUy.coverage.es, /histórico por RUT/)
assert.match(proveedorUy.ux.es, /precios adjudicados/)
assert.ok(proveedorUy.plans.every(plan => plan.currencyInferred !== true))
