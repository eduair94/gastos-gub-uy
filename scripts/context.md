...
| [refresh-pliego-summaries.ts](refresh-pliego-summaries.ts) | Safe audit/refresh for summaries generated by the legacy truncated pipeline. Dry-run by default; `--commit` force-regenerates atomically and preserves the previous cache on provider failure. Supports `--ids=a,b` and `--limit=1..15`. |
